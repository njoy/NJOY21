...
SCENARIO( "LEAPR input",
  ...
  GIVEN( "valid LEAPR input, multiple temperatures loops" ){
    WHEN( "temp specific inputs (Card11-18) provided for both temps" ){
      WHEN( "secondary scattereri not included (card6 nss = 0)" ){
	  ...
	  THEN( "first temp loop invoked, discrete oscillators included" ){
	  ...
	  THEN( "second temp loop invoked, no discrete oscillators" ){
      ...
      WHEN( "secondary scatterer is included (card6 nss = 1)" ){
        iRecordStream<char> iss( std::istringstream(
          "20\n"                                  // Card1
          "'graphite, endf model'\n"              // Card2
          "2 0 101\n"                             // Card3

...
SCENARIO( "ERRORR, Rest, Card8a, Mts",
          "[ERRORR] [Rest] [Card8a] [Mts]" ){
  GIVEN( "valid inputs" ){
    WHEN( "valid inputs are provided" ){
      ...
      THEN( "the values can be verified" ){
        auto mts = argument::extract< ERRORR::Rest::Card8a::Mts >(
                                                        iss, inp.size() ).value;
  ...
  GIVEN( "invalid inputs" ){
    WHEN( "no values are provided" ){
      ...
      THEN( "an exception is thrown" ){
        REQUIRE_THROWS( argument::extract< ERRORR::Rest::Card8a::Mts >(
                                                                     iss, 0 ) );
      }
    } // WHEN

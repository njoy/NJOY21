...
SCENARIO( "ERRORR, Rest, Card12b, Egn",
          "[ERRORR] [Rest] [Card12b] [Egn]" ){
  GIVEN( "valid inputs" ){
    WHEN( "a valid input is provided" ){
      ...
      THEN( "the values can be verified" ){
        auto egn =
                argument::extract< ERRORR::Rest::Card12b::Egn >( iss, 4 ).value;
  ...
  GIVEN( "invalid inputs" ){
    WHEN( "a negative value is provided" ){
      ...
      THEN( "an exception is thrown" ){
        REQUIRE_THROWS( argument::extract< ERRORR::Rest::Card12b::Egn >(
                                                                     iss, 4 ) );
      }
    } // WHEN

  ...
  GIVEN( "valid inputs" ){
    WHEN( "valid inputs are provided" ){
      ...
      THEN( "the values can be verified" ){
        ERRORR::Rest::Card9 card9( iss, 4, 2 );
    ...
    WHEN( "no value is provided" ){
      ...
      THEN( "the default behavior can be verified" ){
        ERRORR::Rest::Card9 card9( iss, 6, 1 );
  ...
  GIVEN( "invalid inputs" ){
    WHEN( "too many values are provided" ){
      ...
      THEN( "an exception is thrown" ){
        REQUIRE_THROWS( ERRORR::Rest::Card9( iss, 2, 3 ) );
      }
    } // WHEN
    ...
    WHEN( "an invalid value is provided" ){
      ...
      THEN( "an exception is thrown" ){
        REQUIRE_THROWS( ERRORR::Rest::Card9( iss, 1, 4 ) );
      }
    } // WHEN
  } // GIVEN
} // SCENARIO

...
SCENARIO( "ERRORR, Rest, Card9, Akxy",
          "[ERRORR] [Rest] [Card9] [Akxy]" ){
  GIVEN( "valid inputs" ){
    WHEN( "valid inputs are provided" ){
      ...
      THEN( "the values can be verified" ){
        auto akxy = argument::extract< ERRORR::Rest::Card9::Akxy >(
                                                              iss, 4, 3 ).value;
    ...
    WHEN( "no input is provided" ){
      ...
      THEN( "the default value can be verified" ){
        for( size_t i = 0; i < num_vals; i++ ){
          ...
          REQUIRE( ref == argument::extract< ERRORR::Rest::Card9::Akxy >(
  ...
  GIVEN( "invalid inputs" ){
    WHEN( "an invalid value is provided" ){
      ...
      THEN( "an exception is thrown" ){
        REQUIRE_THROWS( argument::extract< ERRORR::Rest::Card9::Akxy >(
                                                                  iss, 4, 1 ) );
      }
    } // WHEN

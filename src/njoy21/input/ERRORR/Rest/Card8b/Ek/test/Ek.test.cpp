...
SCENARIO( "ERRORR, Rest, Card8b, Ek",
          "[ERRORR] [Rest] [Card8b] [Ek]" ){
  GIVEN( "valid inputs" ){
    WHEN( "valid values are provided" ){
      ...
      THEN( "the values can be verified" ){
        auto ek = argument::extract< ERRORR::Rest::Card8b::Ek >( iss, 4 ).value;
  ...
  GIVEN( "invalid inputs" ){
    WHEN( "no values are provided" ){
      ...
      THEN( "an exception is thrown" ){
        REQUIRE_THROWS( argument::extract< ERRORR::Rest::Card8b::Ek >(iss, 0) );
      }
    } // WHEN
    ...
    WHEN( "fewer values than expected are provided" ){
      ...
      THEN( "an exception is thrown" ){
        REQUIRE_THROWS( argument::extract< ERRORR::Rest::Card8b::Ek >(iss, 4) );
      }
    } // WHEN

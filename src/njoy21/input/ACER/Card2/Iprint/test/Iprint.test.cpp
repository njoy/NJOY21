...
SCENARIO( "ndir output values",
  ...
  GIVEN( "valid iprint values" ){
    ...
    THEN( "the returned class has the correct value" ){
      for( auto& iprint : validValues ){
        ...
        REQUIRE( 
          iprint == argument::extract< ACER::Card2::Iprint >( iss, ln ).value );
      }
    }
  }
  GIVEN( "no iprint values" ){
    THEN( "the default value is returned" ){
      ...
      REQUIRE( ACER::Card2::Iprint::defaultValue() == 
                argument::extract< ACER::Card2::Iprint >( iss, ln ).value );
    }
  }
  ...
  GIVEN( "invalid iprint values" ){
    ...
    THEN( "an exception is thrown" ){
      for( auto& iprint : invalidValues ){
        ...
        REQUIRE_THROWS( argument::extract< ACER::Card2::Iprint >( iss, ln ) );
      }
    }
  }

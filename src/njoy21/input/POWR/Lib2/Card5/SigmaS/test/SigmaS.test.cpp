...
SCENARIO( "POWR, Lib2, Card5, SigmaS",
          "[POWR] [Lib2] [Card5] [SigmaS]" ){
  GIVEN( "valid inputs" ){
    for( double d : {0.0, 0.1, 1.0, 10.0, 100.0} ){
      ...
      WHEN( situ.c_str() ){
        ...
        THEN( "the value can be verified" ){
          REQUIRE( Approx( d ) == argument::extract< POWR::Lib2::Card5::SigmaS
                                                               >( iss ).value );
        }
      }
    } // WHEN
    ...
    WHEN( "no input is provided" ){
      ...
      THEN( "the default value can be verified" ){
        REQUIRE( Approx( 0.0 ) == argument::extract< POWR::Lib2::Card5::SigmaS
                                                               >( iss ).value );
      }
    } // WHEN
  } // GIVEN
    ...
    for( double d : {-100.0, -10.0, -1.0, -0.1} ){
      ...
      WHEN( situ.c_str() ){
        ...
        THEN( "an exception is thrown" ){
          REQUIRE_THROWS(argument::extract< POWR::Lib2::Card5::SigmaS >( iss ));
        }
      }
    } // WHEN
  } // GIVEN
